...
export class Logger {
  ...
  constructor(options: LoggerOptions = {}) {
    this.prefix = options.prefix || 'E2EWrapper';
    this.colors = options.colors !== false; // Default to true
    
    // Read log level from environment variable
    const envLevel = process.env.E2EWRAPPER_LOG?.toLowerCase();
    switch (envLevel) {
      case 'error':
  ...
  private formatMessage(level: string, message: string, color: keyof typeof this.colorCodes): string {
    ...
    return `${timeStr} ${prefixStr} ${levelStr} ${message}`;
  }

  private log(level: LogLevel, levelName: string, color: keyof typeof this.colorCodes, message: string, ...args: any[]): void {
